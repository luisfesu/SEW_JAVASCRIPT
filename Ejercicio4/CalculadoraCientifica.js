...
class Calculadora {
  constructor(nombre) {
    ...
    document.addEventListener("keydown", (event) => {
      ...
      switch (keyName) {
        case "7":
          calculadora.add(7);
          calculadora.mostrarEnPantalla("screen");
          break;
        case "8":
          calculadora.add(8);
          calculadora.mostrarEnPantalla("screen");
  ...
  operacion(operator) {
    if (
      ...
      this.currentScreenNumber.length <= 0
    ) {
      ...
      this.val1.toString().length <= 0 ||
      (this.val1.toString().length > 0 && this.currentScreenNumber.length > 0)
    ) {
...
class CalculadoraCientifica extends Calculadora {
  ...
  mostrarEnPantalla(idPantalla) {
    document.getElementById(idPantalla).value =
      this.val1 + "\n" + this.currentScreenNumber;
  }

  square() {
    if (this.currentScreenNumber.length > 0) {
      this.currentScreenNumber = Math.pow(
        Number(this.currentScreenNumber),
        2
      ).toString();
    } else if (this.val1.toString().length > 0) {
      this.currentScreenNumber = Math.pow(Number(this.val1), 2).toString();
    }
  }

  squareRoot() {
    if (this.currentScreenNumber.length > 0) {
      this.currentScreenNumber = Math.sqrt(
        Number(this.currentScreenNumber)
      ).toString();
    } else if (this.val1.toString().length > 0) {
      this.currentScreenNumber = Math.sqrt(Number(this.val1)).toString();
    }
  }

  seno() {
    if (this.currentScreenNumber.length > 0) {
      this.currentScreenNumber = Math.sin(
        Number(this.currentScreenNumber)
      ).toString();
    } else if (this.val1.toString().length > 0) {
      this.currentScreenNumber = Math.sin(Number(this.val1)).toString();
    }
  }

  coseno() {
    if (this.currentScreenNumber.length > 0) {
      this.currentScreenNumber = Math.cos(
        Number(this.currentScreenNumber)
      ).toString();
    } else if (this.val1.toString().length > 0) {
      this.currentScreenNumber = Math.cos(Number(this.val1)).toString();
    }
  }

  tangente() {
    if (this.currentScreenNumber.length > 0) {
      this.currentScreenNumber = Math.tan(
        Number(this.currentScreenNumber)
      ).toString();
    } else if (this.val1.toString().length > 0) {
      this.currentScreenNumber = Math.tan(Number(this.val1)).toString();
    }
  }

  delete() {
    if (this.currentScreenNumber.length > 0) {
      this.currentScreenNumber = this.currentScreenNumber.slice(0, -1);
    }
  }
}

var calculadora = new CalculadoraCientifica("Calculadora Básica");
